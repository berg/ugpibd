...
impl<T: NiTransport + 'static> GpibBackend for NiUsbHsBackend<T> {
    ...
    async fn write(&mut self, pad: u8, data: &[u8], send_eoi: bool) -> Result<()> {
        ...
        //
        // Leaving the instrument addressed to talk after a write would remove
        // the ATN edge that costs a first byte on some instruments (see the
        // comment in `read`), but it asks a device to talk when the write was
        // not a query. A 34401A logs -420 Query UNTERMINATED for every one of
        // those and overflows its error queue within a single test run.
        // Restricting it to writes that expect a response means inspecting the
        // payload for a question mark, which is a SCPI concern and has no
        // business in a GPIB transport. Measurements are on the branch
        // `experiment/write-turnaround`.
        Ok(())
    }
    ...
    async fn read(&mut self, pad: u8, max_len: usize) -> Result<(Vec<u8>, bool)> {
        ...
        if self.read_addressing != Some(want) {
            ...
            // Address with ATN and drop to standby in one go, and leave the
            // acceptor alone around it.
            //
            // This adapter arms its read only after ATN has already been
            // released, and its chip will accept a byte in the gap. Whatever
            // is accepted there is discarded when the read operation starts,
            // so an instrument that sources its first byte immediately on the
            // ATN edge loses it — silently, with EOI and a byte count that
            // matches what arrived.
            //
            // That ordering belongs to the adapter's firmware and nothing on
            // this side changes it: the same instrument read through an
            // Agilent 82357 is clean, because a TMS9914 acceptor free-runs and
            // takes the byte, and NI's own driver loses it just as this one
            // does. Rearranging the acceptor around this point has been tried
            // eleven ways against a talker that provokes it and every one is
            // worse; the arms and their rates are in the bench notes, with
            // the caveat that they turned out to measure how many USB round
            // trips precede the standby rather than anything about the
            // acceptor.
            //
            // Do not add timing mitigations. The loss is a race — widening the
            // gap drives it to every read and narrowing it only shifts the
            // odds — so any such change trades one instrument's behaviour
            // against every other instrument's while leaving the loss silent
