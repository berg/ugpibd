...
//
// Vendored from lxi-rs common/messages.rs (GPL-3.0-or-later).
// Adapted to tokio AsyncRead/AsyncWrite.
...
use super::errors::{Error, FatalErrorCode, NonFatalErrorCode};
...
impl Message {
    ...
    pub fn no_payload(self) -> Message {
        Self {
            payload: Vec::new(),
            ..self
        }
        ...
        RD: AsyncRead + Unpin,
    {
        ...
        match MessageType::from_message_type(buf[2]) {
    ...
    pub async fn write_to<WR>(&self, writer: &mut WR) -> Result<(), io::Error>
    where
        WR: AsyncWrite + Unpin,
    {
        let mut buf = [0u8; Message::MESSAGE_HEADER_SIZE];
...
/// Helpers for the session loops: send a fatal/non-fatal error back to the
/// client and (for fatal) bail out of the loop.
pub async fn send_fatal<W: AsyncWrite + Unpin>(
    writer: &mut W,
    code: FatalErrorCode,
...
pub async fn send_nonfatal<W: AsyncWrite + Unpin>(
    writer: &mut W,
    code: NonFatalErrorCode,
    msg: impl Into<String>,
) -> io::Result<()> {
    let m = msg.into();
    tracing::warn!("hislip non-fatal: {m}");
    Message::from(Error::NonFatal(code, m))
        .write_to(writer)
        .await?;
    writer.flush().await
}

/// Message type values from HiSLIP spec Table 4.
...
impl MessageType {
    ...
    pub fn message_params(self, control_code: u8, message_parameter: u32) -> Message {
        Message {
            message_type: self,
            control_code,
            message_parameter,
...
impl InitializeResponseParameter {
    pub fn new(negotiated_protocol: Protocol, session_id: u16) -> Self {
...
impl InitializeResponseControl {
    pub fn new(prefer_overlap: bool, encryption_mode: bool, initial_encryption: bool) -> Self {
...
impl AsyncInitializeResponseParameter {
    pub fn new(server_vendor_id: u16) -> Self {
...
impl AsyncInitializeResponseControl {
    pub fn new(secure_connection: bool) -> Self {
