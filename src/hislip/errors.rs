...
//
// Vendored from lxi-rs common/errors.rs (GPL-3.0-or-later).
...
impl Display for Error {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self {
            Error::Fatal(err, msg) => write!(f, "fatal {} ({msg})", err.error_code()),
            Error::NonFatal(err, msg) => write!(f, "non-fatal {} ({msg})", err.error_code()),
        }
    }
}
